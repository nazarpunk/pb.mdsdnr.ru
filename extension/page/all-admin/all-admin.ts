...
document.querySelectorAll<HTMLLinkElement>(`.grid-view table.items a`).forEach($a => {
    let title = $a.title.trim();
    if (title === `К сведению`) $a.innerHTML = `<img src="images/info.png" alt="${title}">`;
    if ($a.classList.contains(`copy`) || $a.title === `Подписать`) {
        let type: { [key: number]: { need: boolean, complete: boolean, name: string } } = {
