	...
	methods = {
		...
		update: function (options) {
			return this.each(function () {
				...
				} else if (options.data === undefined) options.data = $(settings.filterSelector).serialize();

				if (settings.csrfTokenName && settings.csrfToken) {
					if (typeof options.data == 'string') options.data += '&' + settings.csrfTokenName + '=' + settings.csrfToken;
					else options.data[settings.csrfTokenName] = settings.csrfToken;
				}
