// @ts-nocheck
{
    const onMessage = (request: any, sender: any, sendResponse: any) => {
        const manifest: object = chrome.runtime.getManifest();

        // <editor-fold desc="setClientSidebar">
        if (request.hasOwnProperty(`setClientSidebar`)) {
            chrome.storage.local.get<any>(['token'], result => {
        ...
        if (request.hasOwnProperty(`getClientSidebar`)) {
            chrome.storage.local.get<any>([`ID`, `clients`], result => {
        ...
        if (request.hasOwnProperty(`getClients`)) {
            chrome.storage.local.get<any>(['ID', 'clients', 'token'], result => sendResponse(result));
            return true;
        }
        // </editor-fold>
        ...
        if (request.hasOwnProperty(`getClientSignature`)) {
            let out = {};
            chrome.storage.local.get<any>([`signature`], result => {
